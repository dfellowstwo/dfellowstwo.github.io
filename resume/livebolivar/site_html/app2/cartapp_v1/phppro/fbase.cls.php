...
class FBase {
	...
	function FBase ( $path, $fields, $filenamelen = '' , $dirprefix = '' ) {
		...
		if( ! is_dir( $this->path ) ) {
			$this->_SetErrorMessage( "Path '" . $this->path . " does not exist or is not a directory." );
			return false;
		}
...
/***************************** Public Methods ********************************/
	...
	function RetrieveData ( $route, &$data ) {
		...
		if( empty( $route ) ) {
			$this->_SetErrorMessage( 'Cannot find data if the item code is empty.' );
			return false;
		}
...
/************************ Private Methods (start with a _) ***************************/
	...
	// paging parameters are there for compatibility with hosted version only, they are not used 
	...
	// pagination is only available in hosted version
