...
function testPayPal ( &$message ) {
	...
	if( $dummy === false ) {
		...
		curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 10);
		if( curl_exec($ch) ) {
			$message = "The necessary HTTPS connection to PayPal failed, but a request to e.g. https://www.google.com succeeded. This may indicate that Curl is working correctly but that the ISP has not configured HTTPS correctly. Check with their helpdesk, may be you need to configure a proxy.";
		} else {
