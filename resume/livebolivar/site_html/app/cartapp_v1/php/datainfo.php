...
if( isset( $_POST['code'] ) && $_POST['code'] == md5( 'coffeeshop' . trim( $config['shopname']) ) )
	$auth = true;
else
	$auth = false;
...
<?php
	foreach( $config  as $key => $value ) {
		if( is_array( $value ) &&
			!( $key == 'TaxRates' || $key == 'TaxLocations' || $key == 'ShippingRates' ) )
		{
	...
	if( is_array( $prodrates ) ) {
		...
		foreach( $rownames as $rn ) {
			...
			foreach( $colnames as $cn ) {
				$r[] = $prodrates[$rn][$cn] / pow(10, $config['TaxRatesDecimals']);
			}
			tr( $r );
		}
	} else {
...
<?php
	...
	if( is_array( $config['ShippingRates'] ) ) {
		$rownames = array_keys( $config['ShippingRates'] );
		$colnames = array_keys( $config['ShippingRates'][ $rownames[0] ] );
		...
		foreach( $rownames as $rn ) {
			...
			foreach( $colnames as $cn ) {
				...
				foreach( $config['ShippingRates'][$rn][$cn] as $weight => $value ) {
					...
					if( $weight != -2 ) {
						$tmp .= ' - ' . $value / pow(10, $config['ShippingRatesDecimals']);
					}
				}
				$r[] = $tmp;
			}
...
<table>
<?php
	tr ( array( 'Name', 'Meta-Keywords', 'Meta-Description', 'ID', 'URL', 'Contains Products', 'Parent ID', 'Parent Name', 'Parent HREF', 'Image', 'Image is Set', 'Tile Image', 'Content', 'Contains Sub Cats.' ), 'head' );
...
<h2>Products</h2>
<?php
	$colwidths = array ( 50, 50, 150, 200, 200, 600, 200, 300, 50, 50, 50, 50,
						 20, 50, 20, 20, 20, 50, 50, 50, 20, 20, 50, 50,
						150, 20, 50, 50, 20, 20, 200, 200, 300 );
	echo '<table width="' . array_sum( $colwidths ) . '">';
	col( $colwidths );
	tr ( array( 'ID', 'Group', 'Url', 'Name', 'Short', 'Long', 'Meta-Keywords', 'Meta-Description', 'Weight', 'digits', 'Units', 'Show Weight',
				'Starred', 'Show in Cat', 'Ref.', 'Stock', 'Show Stock', 'Your $', 'Retail $', 'Discount', 'Is %', 'Tax', 'Shipping', 'Handling',
				'Option', 'Force Option', 'Qty Type', 'Qty Default', 'Qty Min', 'Qty Max', 'Img Full', 'Img Small', 'Thumbs'), 'head');
